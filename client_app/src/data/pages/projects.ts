...
export const getProjects = async (options: Sorting & Pagination & Search & Filter) => {
  await sleep(1000)
  ...
  if (options.searchQuery) {
    projects = projects.filter((project) => project.project_name.includes(options.searchQuery))
  }

  if (options.startDate || options.endDate) {
    projects = projects.filter((project) => {
      const creationDate = new Date(project.creation_date)
      if (options.startDate && creationDate < options.startDate) return false
      if (options.endDate && creationDate > options.endDate) return false
      return true
    })
  }
